...
class CustomFlag(BaseFlag):
    ...
    def compare(saved, provided):
        ...
        relevant_element = next((item for item in sources if item["key"] == "first-trip"), None)
        if not relevant_element:
            return False
        
        return request_verdict(client, relevant_element["question"], provided, relevant_element["solution"])
